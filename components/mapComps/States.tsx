...
export default function PolygonComp({dataState,dataset}:compProps) {
  ...
  return (
    <div>
      {mapState.order == 'state' || mapState.order == 'senatorial' || mapState.order == 'federal' ? dataState.features.map((state,index) => {
            ...
            return(
                <State
                    ...
                    dataSet={dataset.filter((result) => {return state.properties.admin1Name == result.loc})[0]}
        ...
        {/* GEOZONES */}
        {mapState.order == 'geo-political-zone' ? dataState.features.filter(state => {return state.properties.geozone == mapState.active.zone}).map((state,index) => {
            const coordinates:any =  state.geometry.coordinates[0].map((item:any) => {
                return [item[1], item[0]]
            })
            return(
